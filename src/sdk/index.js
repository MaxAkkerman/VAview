...
export async function getAct4Info(act4Addr,net) {
    console.log("net,",net)
    const client = new TonClient({network: { endpoints: [net]}});


    const act4acc = new Account(Act4Contract, {address:act4Addr, client});
    let collator = await act4acc.runLocal("collator", {});
    let collatorPhotoLink = await act4acc.runLocal("collatorPhotoLink", {});
    let voteMatrix = await act4acc.runLocal("voteMatrix", {});
    let validator = await act4acc.runLocal("validator", {});
    let countValidators = await act4acc.runLocal("countValidators", {});
    let validatorsFor = await act4acc.runLocal("validatorsFor", {});
    let validatorsAgainst = await act4acc.runLocal("validatorsAgainst", {});
return {...collator.decoded.output,
...collatorPhotoLink.decoded.output,
...voteMatrix.decoded.output,
...validator.decoded.output,
...countValidators.decoded.output,
...validatorsFor.decoded.output,
...validatorsAgainst.decoded.output
    }

// console.log("collator",collator.decoded.output,"collatorPhotoLink",collatorPhotoLink.decoded.output,"voteMatrix",voteMatrix.decoded.output,"validator",validator.decoded.output,"countValidators",countValidators.decoded.output,"validatorsFor",validatorsFor.decoded.output,"validatorsAgainst",validatorsAgainst.decoded.output)

}


export async function getAllDeAudits(net,rootAddress) {
    const client = new TonClient({network: { endpoints: [net]}});
  ...
  struct MunicipalBody {
    ...
    for (const deAudit of raw) {
        ...
        for (const item of districtKeys) {
            ...
            let dItem = {id:item,data:forPush,name:hex2ascii(district[item].name)}
            deAuditData.formated.push(dItem)
